...
export default function MaintenancePlatform() {
  ...
  return <div className="maintenance-platform">
    <nav className="platform-nav" aria-label="Navegación de la plataforma"><a href="/">Mantenimiento LC1C</a>{spares && <span> / Repuestos Críticos</span>}{repairs && <span> / Reparaciones Taller</span>}{(controls || control) && <><span> / </span><a href="/controles-criticos">Controles Críticos</a></>}{control && <span> / {control.title}</span>}</nav>
    <Suspense fallback={<p className="platform-loading" role="status">Cargando módulo…</p>}>
      {spares ? <Spares /> : repairs ? <Repairs /> : control ? <control.Component /> : home || controls ? <main className="platform-home">
