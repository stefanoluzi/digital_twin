...
export function LcoCouplingsScreen({ standalone = false, standaloneThemeControl }: { standalone?: boolean; standaloneThemeControl?: ReactNode }) {
  ...
  return <main className="lco-couplings-screen">
    {standalone ? <header className="lco-standalone-header"><div><small>LACO 1</small><h1>Acoplamientos LCO</h1><span className="lco-local-storage-note" title="Datos compartidos confirmados por el servidor.">● PostgreSQL · datos compartidos</span></div><div className="lco-standalone-actions"><StorageStatus status={maintenance.lcoStorageStatus} error={maintenance.lcoStorageError} /><details className="lco-actions-menu" onKeyDown={(event) => { if (event.key === 'Escape') { event.currentTarget.open = false; event.currentTarget.querySelector('summary')?.focus() } }} onBlur={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) event.currentTarget.open = false }}><summary>Acciones</summary><div className="lco-actions-dropdown" onClick={(event) => { if ((event.target as HTMLElement).closest('button:not(:disabled)')) event.currentTarget.closest('details')?.removeAttribute('open') }}>{standaloneThemeControl}<button onClick={() => void initializeLcoCentral()}>Actualizar datos</button><button onClick={() => runLco(migrateLocalLco)}>Migrar datos locales anteriores</button><button title="Copia completa para recuperar la aplicación, incluidas las fotos" onClick={exportBackup}>Guardar respaldo</button><button title="Informe legible con estado actual e historial" disabled={exportingExcel} onClick={() => void exportExcel()}>{exportingExcel ? 'Generando Excel…' : 'Exportar Excel'}</button><button onClick={() => importRef.current?.click()}>Importar respaldo</button><button onClick={() => setModal({ type: 'SETTINGS' })}>Configurar</button></div></details><button className="primary" onClick={() => setModal({ type: 'INSPECTION', couplingIds: [] })}>+ Registrar inspección</button><input ref={importRef} hidden type="file" accept=".lcocouplings,.json,application/json" onChange={(event) => { void readImport(event.target.files?.[0]); event.target.value = '' }} /></div></header> : <header className="lco-screen-header">
    ...
    <nav className="lco-view-tabs" aria-label="Vistas de Acoplamientos LCO">{([['STATE', 'Estado actual'], ['TRACKING', 'Seguimiento controles'], ['HISTORY', 'Historial']] as const).map(([id, label]) => <button key={id} className={view === id ? 'active' : ''} onClick={() => setView(id)}>{label}</button>)}{standalone && <div className="lco-standalone-reference"><label>Fecha de referencia<input type="date" value={maintenance.referenceDate} onChange={(event) => maintenance.setReferenceDate(event.target.value)} /></label></div>}</nav>
