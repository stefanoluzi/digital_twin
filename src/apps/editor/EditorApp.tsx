...
export function EditorApp() {
  useEffect(() => {
    ...
    const onKeyDown = async (event: KeyboardEvent) => {
      ...
      if (window.confirm(lockedCount ? 'Hay objetos bloqueados. ¿Deseas eliminarlos igualmente?' : selected.length > 1 ? `¿Deseas eliminar los ${selected.length} objetos seleccionados?` : '¿Deseas eliminar el objeto seleccionado?')) { event.preventDefault(); const policies = new Map(); for (const assetId of selected) policies.set(assetId, await requestAssetDeletePolicy(assetId)); deleteAssetsWithMaintenancePolicy(selected, policies) }
    }
