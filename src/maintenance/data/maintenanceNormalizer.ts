...
import { EMPTY_MAINTENANCE_DATA, type MaintenanceData, type MaintenanceEventType, type MaintenanceIntervalUnit, type MaintenanceSource } from '../domain/maintenanceTypes'
...
export function normalizeMaintenanceData(value: unknown): MaintenanceData {
  ...
  return {
    ...
    subassemblies: array(raw.subassemblies).map((item) => ({ id: text(item.id), equipmentId: text(item.equipmentId), name: text(item.name, 'Subconjunto'), description: text(item.description), sapId: text(item.sapId), active: bool(item.active), criticality: ['A', 'B', 'C', 'D'].includes(item.criticality) ? item.criticality : '', source: source(item.source), createdAt: createdAt(item.createdAt) })).filter((item) => item.id && item.equipmentId),
