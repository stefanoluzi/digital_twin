...
describe('legacy normalization', () => {
  it('normaliza datos inexistentes como dominio vacío', () => {
    expect(normalizeMaintenanceData(undefined)).toEqual({ equipment: [], subassemblies: [], plans: [], events: [], units: [] })
  })
  ...
  it('migra trackingMode faltante a REPLACEMENT y conserva v3', () => { const data = normalizeMaintenanceData({ subassemblies: [{ id: 'S', equipmentId: 'E', name: 'Posición' }], units: [] }); expect(data.subassemblies[0].trackingMode).toBe('REPLACEMENT'); expect(data.units).toEqual([]) })
  it('normaliza schema v3 sin degradarlo ni renombrar subassemblies', () => {
    ...
    expect(project.schemaVersion).toBe(3); expect(project.maintenance.subassemblies[0].trackingMode).toBe('SERIALIZED'); expect(project.maintenance.units).toEqual([{ reserved: true }])
  })
})
