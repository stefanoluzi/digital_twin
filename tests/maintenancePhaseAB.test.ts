...
describe('integridad asset-maintenance', () => {
  ...
  it('undo/redo del rename también relinka Maintenance', () => { const linked = { ...data, equipment: [{ ...data.equipment[0], assetId: 'MTR_001' }] }; const scene = useSceneStore.getState(); useMaintenanceStore.getState().loadMaintenance(linked); scene.clearHistory(); scene.selectOnly('MTR_001'); expect(renameAssetAndRelinkMaintenance('MTR_001', 'MTR_UNDO')).toBe(true); expect(useMaintenanceStore.getState().equipment[0].assetId).toBe('MTR_UNDO'); expect(scene.undo()).toBe(true); expect(useMaintenanceStore.getState().equipment[0].assetId).toBe('MTR_001'); expect(scene.redo()).toBe(true); expect(useMaintenanceStore.getState().equipment[0].assetId).toBe('MTR_UNDO'); scene.undo(); scene.clearHistory(); useMaintenanceStore.getState().resetMaintenance() })
  it('delete policy conserva huérfano o elimina la cascada', () => { expect(applyAssetDeletePolicy(data, 'ASSET-1', 'KEEP_ORPHAN')).toEqual(data); const deleted = applyAssetDeletePolicy(data, 'ASSET-1', 'DELETE_MAINTENANCE'); expect(deleted).toEqual({ equipment: [], subassemblies: [], plans: [], events: [], units: [] }) })
  it('undo/redo del delete en cascada restaura ambas partes', () => { const linked = { ...data, equipment: [{ ...data.equipment[0], assetId: 'MTR_001' }] }; const scene = useSceneStore.getState(); useMaintenanceStore.getState().loadMaintenance(linked); scene.clearHistory(); expect(deleteAssetWithMaintenancePolicy('MTR_001', 'DELETE_MAINTENANCE')).toBe(true); expect(useSceneStore.getState().objects.some((item) => item.id === 'MTR_001')).toBe(false); expect(useMaintenanceStore.getState().equipment).toHaveLength(0); expect(useSceneStore.getState().undo()).toBe(true); expect(useSceneStore.getState().objects.some((item) => item.id === 'MTR_001')).toBe(true); expect(useMaintenanceStore.getState().equipment[0].assetId).toBe('MTR_001'); expect(useSceneStore.getState().redo()).toBe(true); expect(useMaintenanceStore.getState().equipment).toHaveLength(0); useSceneStore.getState().resetProject(); useMaintenanceStore.getState().resetMaintenance() })
